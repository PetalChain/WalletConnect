...
var $author$project$Main$disconnectMessage = _Platform_outgoingPort('disconnectMessage', $elm$json$Json$Encode$string);
			...
			case 'DisconnectWallet':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{res: ''}),
					$author$project$Main$disconnectMessage('Disconnect'));
			case 'Send':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{res: ''}),
					$author$project$Main$sendMessage('Connect'));
			default:
				var message = msg.a;
				return (message === 'disconnect') ? _Utils_Tuple2(
					_Utils_update(
						model,
						{isConnected: false, res: ''}),
					...
					_Utils_update(
						model,
						{isConnected: true, res: message}),
					$elm$core$Platform$Cmd$none);
		}
	});
var $author$project$Main$DisconnectWallet = {$: 'DisconnectWallet'};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Wagmi+Metamask Wallet Test')
					])),
				A2(
				$elm$html$Html$p,
				_List_Nil,
				_List_fromArray(
					[
						$elm$html$Html$text(model.res)
					])),
				model.isConnected ? A2(
				$elm$html$Html$button,
				_List_fromArray(
					[
						$elm$html$Html$Events$onClick($author$project$Main$DisconnectWallet)
					]),
				_List_fromArray(
					[
